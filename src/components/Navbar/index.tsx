    ...
    <>
      <aside className="fixed bottom-0 left-0 z-40 w-full lg:w-66 lg:relative lg:border-r lg:border-grey-300">
        <div className="lg:sticky lg:top-0 lg:left-0 lg:overflow-x-hidden">
          <nav className="flex bg-base-white w-full justify-between px-3 xs:px-5 sm:px-8 py-1 shadow-sm lg:flex-col lg:shadow-none lg:bg-transparent lg:px-8 lg:py-6">
            {links.map(link => (
                ...
                <Link href={link.url}>
                  <a
                    className={
                      ...
                      (link.subOptions
                        ? 'lg:border-t lg:border-grey-300 lg:mt-4.5 lg:pt-6 lg:mb-1'
                        : 'lg:mb-0.5')
                    }
                  >
                    <Icon
                      ...
                      color={
                        link.url === pathname
                          ? defaultConfig.theme.colors.action[600]
                          : defaultConfig.theme.colors.grey[500]
                      }
                      name={link.icon}
                    />
                    <span
                      className={
                        ...
                        (link.url === pathname
                          ...
                          : 'text-grey-500 ') +
                        (link.subOptions
                          ? 'lg:text-brand-500 lg:uppercase lg:text-xs lg:tracking-wider'
                          : '')
                      }
                    >
                      {link.title}
                      ...
                      <span
                        className={
                          ...
                          (link.url === pathname
                            ...
                            : 'text-grey-500 ') +
                          (link.subOptions
                            ? 'lg:text-brand-500 uppercase lg:text-xs tracking-wider'
                            : '')
                        }
                      >
                        {link.otherTitle}
